...
def cleanup_test_files():
    ...
    # Remove python files
    for f in glob.glob("tmp_reg_test_3*.py"):
        try:
            os.remove(f)
        except Exception:
            pass
    # Remove side files
    for f in glob.glob("sides/tmp_reg_test_3*.side"):
        try:
            os.remove(f)
        except Exception:
            pass
...
def verify_test_results():
    ...
    py_files = glob.glob("tmp_reg_test_3*.py")
    if len(py_files) == 4:
    ...
    side_files = glob.glob("sides/tmp_reg_test_3*.side")
    if len(side_files) == 3:
